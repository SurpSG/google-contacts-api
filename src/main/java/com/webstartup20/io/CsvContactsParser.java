...
public class CsvContactsParser {
    ...
    private final String NO_YOMI = null;
    ...
    private void readContacts(File file){
        ...
        try(BufferedReader br = new BufferedReader(new FileReader(file))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                currentContact = parseContactRow(currentContact, line);
            }
            contacts.add(currentContact);//add last parsed contact
    ...
    private ContactEntry parseContactRow(ContactEntry contactEntry, String row){
        ...
        if(isEmptyRow(givenName) && isEmptyRow(familyName)){
            contactEntry.addEmailAddress(createEmail(email, contactEntry.getName()));
            return contactEntry;
        }else{
    ...
    private ContactEntry createContact(String givenName, String familyName, String email){
        ContactEntry newContactEntry = new ContactEntry();
        Name name = createName(givenName, familyName);
        newContactEntry.setName(name);
        if(validate(email)){
            newContactEntry.addEmailAddress(createPrimaryEmail(email, name));
        }else{
            System.err.format("Email '%s'(%s) was skipped. Invalid email format.", email, name.getFullName().getValue());
        }
    ...
    private Email createEmail(String emailName){
        Email email = new Email();
        email.setAddress(emailName);
        return email;
    }
    ...
    private Email createPrimaryEmail(String email){
        Email primaryMail = createEmail(email);
        primaryMail.setPrimary(true);
        return primaryMail;
    }
